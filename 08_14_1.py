def count_eval(expression: str, result: bool) -> int:
    ...
    # TOP DOWN RECURSIVE W/o MEMOIZATION
    # Base case
    if len(expression) == 0:
        return 0
    if len(expression) == 1:
    ...
    while i < len(expression):
        ...
        left_t = count_eval(left_exp, True)
        left_f = count_eval(left_exp, False)
        right_t = count_eval(right_exp, True)
        right_f = count_eval(right_exp, False)
