...
def get_sibling_node(node1:Optional[ParentTreeNode]) -> None|ParentTreeNode:
    if not node1 or not node1.parent: return None
    parent = node1.parent
    return parent.left if parent.right == node1 else parent.right
...
def solver_fn(root: Optional[ParentTreeNode], p: Optional[ParentTreeNode], q: Optional[ParentTreeNode]) -> Optional[ParentTreeNode]:
    ...
    # required since below loop that climbs up p, doesnot check for this, starts checking for q's ancestor at p'sibling node
    if is_ancestor(p, q): return p
    if is_ancestor(q, p): return q

    curr = p
    while curr.parent:
        sibling = get_sibling_node(curr)
        if is_ancestor(sibling, q):
            return curr.parent
        curr = curr.parent
