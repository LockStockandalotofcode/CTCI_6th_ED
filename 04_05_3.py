...
def validate_bst(root: TreeNode, min_value=float('-inf'), max_value=float('inf')) -> bool:
    ...
    # the current node needs to be validates against its own bounds only
    # no need of safely assigning left or right val, when we have a base check right at the beginning of the recursive function
    if not (min_value < root.val <= max_value):
        return False

    return validate_bst(root.left, min_value, max_value=root.val) and validate_bst(root.right, min_value=root.val, max_value=max_value)
