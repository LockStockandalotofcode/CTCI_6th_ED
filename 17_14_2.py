...
def smallest_k(arr: list[int], k: int) -> list[int]:
    # max-heap approach : whenever element less than max-heap's top, remove max, insert this 
    if k < 0 or  k > len(arr):
        raise ValueError("k must be between 0 and len(arr)")
    if k == 0 or not arr:
        return []
    # max-heap implemented using default min-heap by negating all values while inserting and retrieval
    # removal doesnt matter
    max_heap = [-x for x in arr[:k]]
    heapq.heapify(max_heap)

    for x in arr[k: ]:
        if x < -max_heap[0]: # to peek at the maximum value
            heapq.heappop(max_heap)
            heapq.heappush(max_heap, -x)

            # slight optimisation
            # does both the above jobs
            # heapq.heapreplace(max_heap, -x) 

    k_smallest = [-x for x in max_heap]
    return k_smallest
