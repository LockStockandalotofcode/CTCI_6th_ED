...
def inorder_successor(node: TreeNodeWithParent):
    ...
    # Case 1 (Right Subtree Exists): Go to node.right, then traverse down as far left as possible (while curr.left: curr = curr.left).

    # Case 2 (No Right Subtree): Climb up parent pointers until you step up from a left child (while parent and parent.right == curr). The parent where you make that left turn is the successor. If you reach None, node was the largest element in the BST.

    # if right subtree exists as a right child, get its leftmost node
    if node.right:
    ...
    else: # successor is some ancestor, not somewhere down this branch
        ancestor = node
