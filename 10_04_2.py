...
def search_listy(listy: Listy, target: int) -> int:
    if listy.element_at(0) == -1:
        return -1
    t_last = get_tentative_size(listy, target)
    # actual bound lies between (t_last // 2, t_last]
    ...
    def binary_search_helper(left: int, right: int) -> int:
        ...
        if target == listy.element_at(mid):
            return mid
        elif listy.element_at(mid) != -1 and target > listy.element_at(mid):
            return binary_search_helper(mid + 1, right)
        else:
            return binary_search_helper(left, mid - 1)

    return binary_search_helper(t_last // 2, t_last )

def get_tentative_size(listy: Listy, target: int) -> int:
    check = 1
    while listy.element_at(check) != -1 and listy.element_at(check) < target:
        check *= 2
