def print_binary(num: float) -> str:
    ...
    # APPROACH 2: directly compare with (decreasing powers of 2 from 1/2) 0.5 (1/2)^1 or 2^(-1), 0.25 (1/2)^2 or 2^(-2), 0.125 (1/2)^3 or 2^(-3), 0.625 (1/2)^4 or 2^(-4), gets next digit 0 or 1, then we append it to the result
    if num >= 1 or num <= 0:
        return "ERROR"
    ...
    while num > 0:
        ...
        # compare with decreasing powers of 1/2
        
        # fraction, decreasing powers of 2, from 2 ^ (-1) onwards
        p = 0.5 ** (i)
        if num >= p:
            result.append("1")
            num -= p
