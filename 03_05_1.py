def sort_stack(input_stack:list[int]) -> list[int]:
    ...
    # result is with elements in decreasing order from top to bottom, at end we just pop from this and put in input_stack
    #  smallest element goes to bottom, biggest element goes to top, and 
    # for the ones that fall somewhere in between, we pop from result stack until a smaller element is found, then add current element and followed by putting all elements from temporarily stored stack

    while input_stack:
        curr = input_stack.pop()
        while result and result[-1] > curr:
