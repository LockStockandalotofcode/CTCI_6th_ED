# CODE / LOGIC
# Attempt 4 bottom up rolling variables, state reduction -> space optimisation
# techinqiue called rolling variables, or variable swapping similar to sliding window
# at each new level of execution we keep only the last 3 variables as determined by the recursion formula
...
def triple_step_top_down(n: int):
    ...
    # these early returns are required, otherwise for eg. n = 0, list assignment dp[1] = 1 throws indexErroe
    # absolute base cases;
    if n == 0:
        return 1
    if n == 1:
