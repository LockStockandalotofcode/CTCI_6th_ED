def sparse_search(array: list[str], target: str) -> int:
    ...
    def binary_search_helper(left: int, right: int) -> int:
        ...
        mid_orig = left + (right - left) // 2
        mid = mid_orig

        while mid >= left and array[mid] == "":
            # mid >= left, is the correct choice
            # mid > left, doesn't check if element at left is empty or nonempty
            # consequently, mid = left - 1, when left to mid, both inclusive are empty strings ""
            # so below check executes in that case
            mid -= 1

        if mid < left:
            return binary_search_helper(mid_orig + 1, right)
        # if entire left to mid is full of empty strings, discard the entire range, and move to right half
