...
def fca_helper(root: Optional[TreeNode], p: Optional[TreeNode], q: Optional[TreeNode]) -> Optional[TreeNode]:
    ...
    q_on_right_result = covers(root.right, q)

    # covering all 4 cases of 2 x 2 Truth Table

    # APPROACH 1
    # if p_on_left_result and q_on_right_result:
    #     return root
    # if not p_on_left_result and not q_on_right_result:
    #     return root

    # APPROACH 2
    if p_on_left_result == q_on_right_result:
        return root

    if p_on_left_result and not q_on_right_result:
        return fca_helper(root.left, p, q)
    if not p_on_left_result and q_on_right_result:
