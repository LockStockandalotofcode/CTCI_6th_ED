def sort_stack(input_stack:list[int]) -> list[int]:
    ...
    # input_stack acts as the temporary stack, while inserting a new element into temp_stack, which is sorted all the time
    temp_stack = []

    # temp_stack is with elements in increasing order from top to bottom (just as the solution requires us to do), 
    ...
    # for the ones that fall somewhere in between, we pop from temp_stack stack until a larger element is found, then add current element and followed by putting all elements from temporarily stored stack (input_stack itself in this case)

    # strictly one extra stack for space

    while input_stack:
        curr = input_stack.pop()
        while temp_stack and curr < temp_stack[-1]:
