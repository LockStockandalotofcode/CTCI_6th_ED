    ...
    if abs(len(s1) - len(s2)) == 0:
        ...
        for index_ptr in range(len(s1)):
            if s1[index_ptr] != s2[index_ptr]:
                replacement_count += 1
                if replacement_count > 1: return False
        return True

    if abs(len(s1) - len(s2)) == 1:
        # check for insertion or removal
        # choosing the longer string, reduces the problem to removal from longer string's perspective and insertion (from smaller string's prespective)
        longer = s1 if len(s1) > len(s2) else s2
        shorter = s1 if len(s1) < len(s2) else s2
        l_ptr, s_ptr = 0, 0

        removal_count = 0
        while s_ptr < len(shorter):
            if longer[l_ptr] != shorter[s_ptr]:
                removal_count += 1
                if removal_count > 1: return False
                l_ptr += 1
            else:
                l_ptr += 1
                s_ptr += 1
