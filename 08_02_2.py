def get_path(matrix: list[list[int]]) -> int:
    ...
    path = []
    # maintain a set of dead end cells, this implies they have been visited and path from topleft to bottomright doesnot exist through this cell
    dead_ends = set()

    def find_path(r: int, c: int) -> bool:
        # if cell is offlimit or the cell goes out of boundary of matrix
        if not matrix[r][c] or r < 0 or c < 0: return False
        if (r, c) in dead_ends: return False
