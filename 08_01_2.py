# CODE / LOGIC
# attempt 2
# attempt 1: failed lack of memoization
...
#  very complex for me : using global variable to retain state and avoid re-initialisation in recursive calls
# earlier done - bytebytego; using helper inside function for recursion, and dict for easy lookup, not relying on indices for lookup
def triple_step_top_down(no_of_stairs: int):
    memo = {}
    memo[0] = 1
    memo[1] = 1
    memo[2] = 2
    def helper(n: int):
        ...
        value = helper(n - 3) + helper(n - 2) + helper(n - 1)
        memo[n] = value
        return memo[n]
    
    return helper(no_of_stairs) 
...
def run_tests(triple_step_function):
    ...
    test_cases = [
        ...
        {"category": "Performance", "desc": "35 steps (Verifies O(N) DP / Memoization efficiency)", "n": 35, "expected": 1132436852}
    ]
